...
public class Player : MonoBehaviour
{
        ...
        Configuration boardConfiguration)
    {
        ...
        while (nodeList.Count > 0)
        {
            ...
            //set depth of children nodes
            int childDepth = 1;
            MinimaxTreeNode<Configuration> tempNode = currentNode;
        ...
        bool maximizing)
    {
        ...
        // assign value to this easy situation (only two bears left) 
        //so the computer cannot make an overtly stupid move
        else if (node.Value.NumBears == 2)
        {
            if (maximizing)
            {
                node.MinimaxScore = 1;
        ...
        //if there is only one bin that contains any teddies, then a winning move
        //can easily be made on that node (i.e. take all but one teddy from last bin)
        else if (node.Value.NonEmptyBins.Count == 1)
        {
            if (maximizing)
            {
                node.MinimaxScore = 1;
